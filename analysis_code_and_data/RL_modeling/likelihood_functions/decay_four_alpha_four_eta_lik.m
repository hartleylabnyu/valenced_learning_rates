...
function [lik, latents] = decay_four_alpha_four_eta_lik(block, choices, keys, rewards, x, priors)
...
eta_pos_risk_good = x(7);
eta_neg_risk_good = x(8);
eta_pos_risk_bad = x(9);
eta_neg_risk_bad = x(10);
...
for trial = 1:length(choices)
    ...
    if PE > 0
        if block(trial) == 1
            eta = eta_pos_risk_good;
            alpha_init = alpha_init_pos_risk_good;
        else
            eta = eta_pos_risk_bad;
            alpha_init = alpha_init_pos_risk_bad;
        end
    elseif PE < 0
        if block(trial) == 1
            eta = eta_neg_risk_good;
            alpha_init = alpha_init_neg_risk_good;
        else
            eta = eta_neg_risk_bad;
            alpha_init = alpha_init_neg_risk_bad;
        end
    end
...
lik= lik+log(pdf('beta', eta_pos_risk_good, 1.1, 1.1));
lik= lik+log(pdf('beta', eta_neg_risk_good, 1.1, 1.1));
lik= lik+log(pdf('beta', eta_pos_risk_bad, 1.1, 1.1));
lik= lik+log(pdf('beta', eta_neg_risk_bad, 1.1, 1.1));
