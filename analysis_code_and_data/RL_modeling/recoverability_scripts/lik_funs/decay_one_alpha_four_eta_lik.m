...
function [lik, latents] = decay_one_alpha_four_eta_lik(block_order, block, choices, rewards, x, priors)

beta = x(1);
choice_c = x(2);
alpha_init = x(3);
eta_pos_risk_good = x(4);
eta_neg_risk_good = x(5);
eta_pos_risk_bad = x(6);
eta_neg_risk_bad = x(7);
...
for trial = 1:length(choices)
   ...
   %determine trial num
    ...
    alpha_pos = alpha_init / (1 + eta_pos * (trial_num-1));
    alpha_neg = alpha_init / (1 + eta_neg * (trial_num-1));
