...
# Model comparison
```{r read in aics}
aic_name <- "RL_modeling/output/aics.csv"
...
mean_age_group_aic <- aic_long %>%
  group_by(age_group, model) %>%
  summarize(mean_aic = mean(aic))
```

## Mean aic values
```{r mean aic plot, fig.height = 4, fig.width = 3}
aic_overall_plot <- ggplot(mean_aic, aes(x = model, y = mean_aic)) +
...
## Mean aic values across age groups
```{r aic age group plot, fig.height = 4, fig.width = 5}
aic_age_group_plot <- ggplot(mean_age_group_aic, aes(x = model, fill = age_group, y = mean_aic)) +
...
model_params_name <- "RL_modeling/output/model_fits/four_LR.csv"
