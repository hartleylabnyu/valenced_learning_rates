...
%simulated data
%load('../output/sim_sticky_models_randParams');
load('../output/sim_4LR_sticky');

%model fits
%load('../output/fits_sticky_models_randParams');
load('../output/fits_4LR_sticky');

%% Get simulated and recovered parameters from each model

sim_params = zeros(size(model_fits(1,1).results.params));
fit_params = zeros(size(model_fits(1,1).results.params));

for s = 1:length(sim_data(1).sub_data)
    sim_params(s, :) = sim_data(1).sub_data(s).params;
    fit_params(s, :) = model_fits(1, 1).results.params(s, :);
end
%% Plot
figure;
n_params = sim_data(1).n_params;
param_names = sim_data(1).param_names;
...
writetable(sim_params_table, '../output/fourLR_sticky_sim_params.csv');
writetable(fit_params_table, '../output/fourLR_sticky_fit_params.csv')
