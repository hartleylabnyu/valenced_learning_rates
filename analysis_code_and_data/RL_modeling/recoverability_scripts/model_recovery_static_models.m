...
%% Load data %%
load('../output/mat_files/fits_sticky_models_randParams');

%% Extract AICs for each data set and each model %%
% use only first 3 sim models and first 3 fit models
...
aic_table_stacked = stack(aic_table, 1:3,  'NewDataVariableName','Recovered Model',...
          'IndexVariableName','Simulated Model');
bic_table = array2table(best_models_bic, 'VariableNames', model_name);
bic_table_stacked = stack(bic_table, 1:3,  'NewDataVariableName','Recovered Model',...
          'IndexVariableName','Simulated Model');      
...
%% export bics
writetable(bic_table_stacked, '../output/model_recoverability/bic_recovery_static_sticky.csv');
writetable(aic_table_stacked, '../output/model_recoverability/aic_recovery_static_sticky.csv');
