...
## Model: MaRs accuracy and age with gender
```{r mars accuracy and age with gender model}

lm(mars_acc_scaled ~ age_scaled * gender, mars_data) %>%
  tidy() %>%
  kable()%>%
  kable_classic(full_width = F, html_font = "Arial") %>%
  kable_styling("striped", position = "float_left")
...
```{r plot choice reaction times}
#determine each participant's mean rt
rt_data <- learning_data %>%
  filter(task_part == "real_trial") %>% 
  group_by(sub_id, block_type) %>%
  mutate(trial = rank(trial_index),
         trial_group = ceiling(trial/20)) %>%
  ungroup() %>%
  group_by(sub_id, block_type, trial_group, age_group, block_number) %>%
  summarize(mean_sub_rt = mean(rt, na.rm = T)) 

#save each participant's average RT
sub_rt_means <- rt_data %>%
  group_by(sub_id, block_type) %>%
  summarize(mean_rt = mean(mean_sub_rt))

#save for further analyses in modeling script
#write_csv(sub_rt_means, 'data/other_data_files/rt_means.csv')

#determine mean rt within age groups
age_group_rt <- rt_data %>% 
  group_by(block_type, trial_group, age_group, block_number) %>%
  summarize(mean_rt = mean(mean_sub_rt, na.rm = T),
            se_rt = se(mean_sub_rt))

#plot
rt_plot <- ggplot(age_group_rt, aes(x = trial_group, y = mean_rt, color = factor(block_number))) +
  facet_grid(cols = vars(age_group), rows = vars(block_type)) +
  scale_color_manual(values = c(color1, color2), name = "Block Number") +
  ylab("Mean RT (ms)") +
  xlab("Trial Group (20 trials)") +
  geom_line() +
  geom_point() + 
...
## Model: Reaction times by age, trial, block type and block number
```{r reaction time model, include = F}
#prep data
rt_data <- learning_data %>%
  filter(task_part == "real_trial") %>% 
  group_by(sub_id, block_type) %>%
  mutate(trial = rank(trial_index)) %>%
  filter(rt > 0) #remove NA

rt_data$age_scaled <- scale_this(rt_data$age)
rt_data$block_number <- factor(rt_data$block_number)
rt_data$trial_scaled <- scale_this(rt_data$trial_index)


#run model
rt_model <- mixed(rt ~ age_scaled * trial_scaled * block_type * block_number + 
                          (trial_scaled * block_number |sub_id), 
                          data = rt_data,
                          method = "S",
                          control = lmerControl(optimizer = "bobyqa", optCtrl=list(maxfun=1e6)))

#does not converge w/ both block_type and block_number in RE

```

```{r display rt model output}
nice(rt_model)
...
# trial
# block number
# age x trial x block type x block number
```
...
## Motor perseveration
```{r motor perseveration plot}
#do participants choose the same location after a win?
motor_persev_data <- learning_data %>%
  filter(task_part == "real_trial") %>% 
  group_by(sub_id, block_type) %>%
  mutate(trial = rank(trial_index),
         trial_group = ceiling(trial/20),
         prev_position = lag(choice_position),
         stay = case_when(prev_position == choice_position ~ 1,
                          prev_position != choice_position ~ 0),
         prev_reward = lag(points_earned),
         prev_win = case_when(prev_reward > 0 ~ 1,
                              prev_reward < 0 ~ 0))


sub_persev <- motor_persev_data %>%
  group_by(sub_id, age_group, prev_win) %>%
  summarize(sub_stay_prob = mean(stay, na.rm = T)) %>%
  drop_na() 

persev_scores <- sub_persev %>%
  pivot_wider(names_from = prev_win,
              values_from = sub_stay_prob) %>%
  mutate(persev_score = `1` - `0`) %>%
  select(sub_id, persev_score)

#save
#write_csv(persev_scores, 'data/other_data_files/persev_scores.csv')

age_group_persev <- sub_persev %>%
  group_by(age_group, prev_win) %>%
  summarize(stay_prob = mean(sub_stay_prob, na.rm = T),
            se_stay_prob = se(sub_stay_prob))


motor_persev_plot <- ggplot(age_group_persev, aes(fill = as.factor(prev_win), y = stay_prob, x = age_group)) +
  scale_fill_manual(values = c(color1, color2), name = "Previous Outcome Type",
                    labels = c("Lose", "Win")) +
  #scale_x_discrete(labels = c("Lose", "Win")) +
  ylab("Proportion of Repeated Responses") +
  xlab("Age Group") +
...
## Model: Motor perseveration by previous win and age
```{r motor perseveration model}
#prep data
motor_persev_data <- motor_persev_data %>%
  select(sub_id, age, stay, prev_win) %>%
  drop_na

#prep variables
motor_persev_data$age_scaled <- scale_this(motor_persev_data$age)
motor_persev_data$prev_win <- factor(motor_persev_data$prev_win)

#run model
motor_persev_model <- mixed(stay ~ age_scaled * prev_win + 
                          (prev_win |sub_id), 
                          data = motor_persev_data,
                          family = "binomial", 
                          method = "LRT",
                          control = glmerControl(optimizer = "bobyqa", optCtrl=list(maxfun=1e6)))
```

```{r display motor persev model output}
nice(motor_persev_model)
...
## Optimal choices across block types by continous age
```{r optimal choices continuous age}
# determine each participant's proportion of optimal choices within each trial group
optimal_choices_subs <- optimal_choices %>%
  group_by(sub_id, age, block_type) %>%
  ungroup() %>%
  group_by(sub_id, age, block_type) %>%
  summarize(prop_optimal = mean(optimal_choice, na.rm = T))

#plot
optimal_choice_age_plot <- ggplot(optimal_choices_subs, aes(x = age, y = prop_optimal, color = block_type)) +
  geom_point(size = 1) +
  geom_smooth(method = "lm", aes(fill = block_type)) +
  scale_color_manual(values = c(color1, color2),  name = "Block Type") +
  scale_fill_manual(values = c(color1, color2),  name = "Block Type") +
...
## Model: Optimal choices across ages, trials, block types, and blocks (1st vs. 2nd) with gender
```{r optimal choices w/ block order regression with gender, include = F}
#prep data
optimal_choice_data <- optimal_choices %>%
  group_by(sub_id, block) %>%
  mutate(trial = rank(trial_index)) %>%
  ungroup() %>%
  select(sub_id, optimal_choice, age, gender, trial, block_type, block_number) %>%
  drop_na() %>%
  mutate(trial_scaled = scale_this(trial),
         age_scaled = scale_this(age),
         block_number = factor(block_number))
...
#run model
opt_choice_model.gender <- mixed(optimal_choice ~ age_scaled * trial_scaled * block_type * block_number * gender + 
                          (trial_scaled * block_type|sub_id), 
                          data = optimal_choice_data,
                          family = "binomial", 
                          method = "LRT",
...
```{r display opt choice model with gender output}
nice(opt_choice_model.gender)
```
