...
function [lik, latents] = decay_two_alpha_two_eta_lik(block, choices, keys, rewards, x, priors)
...
for trial = 1:length(choices)
    ...
    %Determine alpha
    if PE > 0
        alpha = alpha_init_pos / (1 + eta_pos * (trial_num-1));
    elseif PE < 0
        alpha = alpha_init_neg / (1 + eta_neg * (trial_num-1));
    end
