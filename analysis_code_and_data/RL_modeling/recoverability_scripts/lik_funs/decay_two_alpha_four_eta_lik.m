...
function [lik, latents] = decay_two_alpha_four_eta_lik(block_order, block, choices, rewards, x, priors)

beta = x(1);
choice_c = x(2);
alpha_init_pos = x(3);
alpha_init_neg = x(4);
eta_pos_risk_good = x(5);
eta_neg_risk_good = x(6);
eta_pos_risk_bad = x(7);
eta_neg_risk_bad = x(8);
...
for trial = 1:length(choices)
    ...
    alpha_pos = alpha_init_pos / (1 + eta_pos * (trial_num-1));
    alpha_neg = alpha_init_neg / (1 + eta_neg * (trial_num-1));
...
if (priors)
    lik= lik+log(pdf('beta', alpha_init_pos, 1.1,1.1));
    lik= lik+log(pdf('beta', alpha_init_neg, 1.1,1.1));
